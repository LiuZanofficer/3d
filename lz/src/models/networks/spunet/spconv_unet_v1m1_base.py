...
def _resolve_gn_num_groups(num_channels: int, preferred_groups: int = 32) -> int:
    candidate_groups = [preferred_groups, 16, 8, 4, 2, 1]
    for num_groups in candidate_groups:
        if num_groups <= num_channels and num_channels % num_groups == 0:
            return num_groups
    return 1


def _build_norm_fn(norm_type: Literal["bn", "gn"]) -> Callable[[int], nn.Module]:
    if norm_type == "bn":
        return partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
    if norm_type == "gn":
        return lambda channels: nn.GroupNorm(
            num_groups=_resolve_gn_num_groups(channels),
...
class SpUNetBottleneck(SpUNetBase):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(in_channels, out_channels, norm_type=norm_type)
        assert len(layers) % 2 == 0
        assert len(layers) == len(channels)
        self.in_channels = in_channels
        self.out_channels = out_channels
