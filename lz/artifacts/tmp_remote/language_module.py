...
class DenseLanguageLitModule(LitModuleBase):
    ...
    def setup(self, stage: str) -> None:
        ...
        for i, val_dataloader in enumerate(val_dataloaders):
            ...
            self.val_datasets[postfix] = dataset
    ...
    def on_validation_epoch_start(self):
        ...
        for postfix in self.val_class_info.keys():
            ...
            for key in metrics.keys():
                metrics[key].reset()
            self._scene_ptr[postfix] = 0

    def _resolve_scene_names(self, batch, postfix: str, batch_size: int):
        scene_names = batch.get("scene_name", None)
        if isinstance(scene_names, tuple):
            scene_names = list(scene_names)
        if isinstance(scene_names, list) and len(scene_names) == batch_size:
            self._scene_ptr[postfix] = int(self._scene_ptr.get(postfix, 0)) + batch_size
            return [str(x) for x in scene_names]

        ptr = int(self._scene_ptr.get(postfix, 0))
        dataset = self.val_datasets.get(postfix)
        if dataset is not None and hasattr(dataset, "scene_names"):
            names = [str(dataset.scene_names[(ptr + i) % len(dataset.scene_names)]) for i in range(batch_size)]
        else:
            names = [f"{postfix}_{ptr + i:06d}" for i in range(batch_size)]
        self._scene_ptr[postfix] = ptr + batch_size
        return names

    def _export_pred_labels(self, batch, preds, segment_fg, postfix: str) -> None:
        pred_root = os.environ.get("SAVE_PRED_LABEL_DIR", None)
        if pred_root is None:
            return
        if int(getattr(self, "global_rank", 0)) != 0:
            return

        save_dir = os.path.join(pred_root, postfix)
        os.makedirs(save_dir, exist_ok=True)

        offset = batch["offset"]
        if isinstance(offset, torch.Tensor):
            offset_list = offset.detach().cpu().tolist()
        else:
            offset_list = list(offset)
        batch_size = len(offset_list) - 1
        scene_names = self._resolve_scene_names(batch, postfix, batch_size)

        preds_cpu = preds.detach().cpu()
        gt_cpu = segment_fg.detach().cpu()
        for i in range(batch_size):
            st, ed = int(offset_list[i]), int(offset_list[i + 1])
            scene_name = os.path.basename(str(scene_names[i]))
            np.save(os.path.join(save_dir, f"{scene_name}.pred.npy"), preds_cpu[st:ed].numpy())
            np.save(os.path.join(save_dir, f"{scene_name}.gt.npy"), gt_cpu[st:ed].numpy())
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self._export_pred_labels(batch, preds, segment_fg, postfix)
